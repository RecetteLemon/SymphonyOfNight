...
void objTest::update(void)
{
	...
	if (IntersectRect(&temp, &_rc, &_obj->getRect()))
	{
		...
		{
			EFFECTMANAGER->play("ObjBroken", _obj->getRect().left + 10, _obj->getRect().top + 10);
		}
	...
	if (IntersectRect(&temp, &_rc, &_obj1->getRect()))
	{
		...
		{
			EFFECTMANAGER->play("ObjBroken", _obj1->getRect().left + 10, _obj1->getRect().top + 10);
		}
	...
	if (IntersectRect(&temp, &_rc, &_obj2->getRect()))
	{
		...
		{
			EFFECTMANAGER->play("ObjBroken", _obj2->getRect().left + 10, _obj2->getRect().top + 10);
		}
