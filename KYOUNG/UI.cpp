...
void UI::render(const char* topBar, const char* backBar,const char* magic)
{
	...
	else IMAGEMANAGER->findImage("앞게이지2")->render(getMemDC(), 60, 10, 0, 0, _width,10);//, _currentFrameX, 0);
...
{
	_width = (currentGauge / maxGauge) * 100;//progressBottom->getWidth();
